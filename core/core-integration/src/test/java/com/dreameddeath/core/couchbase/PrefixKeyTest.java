...
public class PrefixKeyTest {
    ...
    public static class TestPrefixKeyDao extends CouchbaseDocumentDao<TestPrefixKey> {
        ...
        @Override
        public Single<TestPrefixKey> asyncBuildKey(ICouchbaseSession session, TestPrefixKey newObject) throws DaoException {
    ...
    public void testPrefixKey()throws Exception{
        ...
        session.toBlocking().blockingSave(testClass);
        ...
        session.reset();
        TestPrefixKey readClass = session.toBlocking().blockingGet(testClass.getMeta().getKey(),TestPrefixKey.class);
