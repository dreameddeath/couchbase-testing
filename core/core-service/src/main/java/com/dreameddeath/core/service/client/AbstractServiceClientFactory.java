...
public abstract class AbstractServiceClientFactory<TCLIENT extends IServiceClient<?>,TSPEC,TDESCR extends CuratorDiscoveryServiceDescription<TSPEC>> {
    ...
    public void registarClient(TCLIENT client){
        ...
        instanceInfo.setServiceTechType(serviceDiscoverer.getServiceTechnicalType());
        try {
    ...
    public TCLIENT getClient(final String serviceFuncType,final String serviceName, final String serviceVersion){
        return serviceClientMap.computeIfAbsent(ServiceNamingUtils.buildServiceFullName(serviceFuncType,serviceName, serviceVersion),
                serviceFullName -> {
                    try {
