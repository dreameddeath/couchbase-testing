 ...
 */
public class ContextClientFilter implements ClientRequestFilter {
    private IGlobalContextFactory factory;

    @Autowired
    public void setGlobalContextFactory(IGlobalContextFactory factory){
    ...
    public void filter(ClientRequestContext containerRequestContext) throws IOException {
        ...
        if(context!=null){
            containerRequestContext.getHeaders().add(ContextServerFilter.HTTP_CONTEXT_HEADER,factory.encode(context));
        }
    }
}
