...
public class ViewAsyncQueryResult<TKEY,TVALUE,TDOC extends CouchbaseDocument> implements IViewAsyncQueryResult<TKEY,TVALUE,TDOC> {
    ...
    public Observable<JsonObject> getErrorInfo(){
        return RxJavaWrapper.toRxJava2Observable(result.error());
    }

    @Override public Observable<IViewQueryRow<TKEY, TVALUE, TDOC>> getRows() {return RxJavaWrapper.toRxJava2Observable(result.rows()).map(asyncViewRow-> query.getDao().map(asyncViewRow));}
