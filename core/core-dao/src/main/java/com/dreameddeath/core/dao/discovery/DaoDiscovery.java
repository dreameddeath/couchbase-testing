 ...
 */
public class DaoDiscovery extends StandardCuratorDiscoveryImpl<DaoInstanceInfo> {
    ...
    public Set<String> getDomains(){
        return getList().stream().map(DaoInstanceInfo::getDomain).collect(Collectors.toSet());
    }

    public List<DaoInstanceInfo> getList(String domain){
        return getList().stream().filter(daoInfo->domain.equals(daoInfo.getDomain())).collect(Collectors.toList());
    }
