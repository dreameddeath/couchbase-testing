...
public abstract class CouchbaseViewDao<TKEY,TVALUE,TDOC extends CouchbaseDocument> {
    ...
    public IViewQueryRow<TKEY,TVALUE,TDOC> map(AsyncViewRow row,final String keyPrefix){
        try{
            ...
            return new IViewQueryRow<TKEY, TVALUE, TDOC>() {
                ...
                @Override public Single<TDOC> getDoc(ICouchbaseSession session) {return session.asyncGet(docKey);}
    ...
    public Single<IViewAsyncQueryResult<TKEY,TVALUE,TDOC>> asyncQuery(ICouchbaseSession session, boolean isCalcOnly, IViewQuery<TKEY,TVALUE,TDOC> query){
