...
public abstract class BucketDocument<T extends CouchbaseDocument> implements Document<T> {
    ...
    public void syncMeta(BucketDocument<T> ref){
        //Keep orig cas for cache updates
        ref.getDocument().getBaseMeta().setUpdatedFromCas(doc.getBaseMeta().getCas());
        doc.getBaseMeta().setUpdatedFromCas(ref.doc.getBaseMeta().getUpdatedFromCas());
        ...
        doc.getBaseMeta().setDbData(ref.doc.getBaseMeta().getDbData());
