...
public class JobProcessClientDiscovery extends StandardCuratorDiscoveryImpl<JobExecutorClientInfo> {
    ...
    public Set<String> getDomains(){
        return getList().stream().map(JobExecutorClientInfo::getDomain).collect(Collectors.toSet());
    }

    public List<JobExecutorClientInfo> getList(String domain){
        return getList().stream().filter(jobInfo->domain.equals(jobInfo.getDomain())).collect(Collectors.toList());
    }
}
