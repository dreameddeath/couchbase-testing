...
public interface IJobExecutorClient<T extends AbstractJob> {
    ...
    Observable<JobContext<T>> executeJob(T job, IUser user);
    Observable<JobContext<T>> submitJob(T job, IUser user);
    Observable<JobContext<T>> resumeJob(T job, IUser user);
    Observable<JobContext<T>> cancelJob(T job, IUser user);
}
