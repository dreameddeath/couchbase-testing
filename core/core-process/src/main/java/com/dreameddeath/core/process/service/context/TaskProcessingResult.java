...
public class TaskProcessingResult<TJOB extends AbstractJob,T extends AbstractTask>{
    ...
    public static <TJOB extends AbstractJob,T extends AbstractTask> Single<TaskProcessingResult<TJOB,T>> build(TaskContext<TJOB,T> ctxt, boolean needSave){
        return Single.just(new TaskProcessingResult<>(ctxt,needSave));
    }
}
