 ...
 */
public class JobExecutorClientRegistrar extends CuratorRegistrarImpl<JobExecutorClientInfo> {
    public static String getRootPath(){
        try {
            return ProcessConfigProperties.JOB_REGISTER_PATH.getMandatoryValue("The jobClient registering path isn't set");
        }
    ...
    public JobExecutorClientRegistrar(CuratorFramework curatorFramework,String daemonUid,String webServerUid) {
        super(curatorFramework, getRootPath());
        this.daemonUid = daemonUid;
        this.webServerUid= webServerUid;
    }

    public void enrich(JobExecutorClientInfo obj){
        obj.setDaemonUid(daemonUid);
        obj.setWebServerUid(webServerUid);
    }

    @Override
    protected byte[] serialize(JobExecutorClientInfo obj) throws Exception {
