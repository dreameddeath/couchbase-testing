...
public class JsonGrammarTest {
    ...
    public void simpleGrammarText(){
        ...
        for(DatasetObjectNode objectNode:dataset.getElements().get(0).getObject().getNodes()){
            ...
            assertEquals(5, path.getParts().size());
            assertEquals("toto",path.getParts().get(0).getPath());
            assertEquals("[0]",path.getParts().get(1).getPath());
            assertEquals("utut",path.getParts().get(2).getPath());
            assertEquals("[1..-1]",path.getParts().get(3).getPath());
            assertEquals("ta\"ta",path.getParts().get(4).getPath());
