...
public class ProcessorClientPreInit {
    ...
    public Map precreateJobAndTasksClients(Map<Class<? extends AbstractJob>,Collection<Class<? extends  AbstractTask>>> taskClassesMap){
        for(Map.Entry<Class<? extends AbstractJob>,Collection<Class<? extends  AbstractTask>>> entry:taskClassesMap.entrySet()){
            precreateJobClient(entry.getKey());
            fullMap.get(entry.getKey()).addAll(entry.getValue());
        }
        return fullMap;
    }
