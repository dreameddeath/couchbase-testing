...
public class RestLocalWebServerAdminService {
    ...
    @Produces({ MediaType.APPLICATION_JSON })
    @Path("{name}/metrics")
    @ApiOperation(value = "give metrics on a given webserver", response = MetricRegistry.class)
    ...
    @Produces({MediaType.APPLICATION_JSON})
    @Path("{name}/plugins")
    @ApiOperation(value = "give list of plugins on a given webserver", response = PluginCoreInfo.class)
